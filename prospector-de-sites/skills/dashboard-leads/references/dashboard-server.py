...
CAMPOS = ['slug','nome','nicho','cidade','nota','avaliacoes','email','telefone','whatsapp',
          'canalContato','siteAntigo','motivo','status','urlNova','dataProposta','valor','obs',
...
def conexao():
    ...
    for col, tipo in [('contratoStatus',"TEXT DEFAULT 'pendente'"),('contratoEm','TEXT'),('manutencao','REAL'),('pago','INTEGER DEFAULT 0'),('docCliente','TEXT'),('endCliente','TEXT'),('canalContato',"TEXT DEFAULT 'email'")]:
...
class App(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path.split('?')[0] == '/api/config':
            ...
            rw = dict(cfg.get('railway', {}))
            rw['tokenDefinido'] = bool(rw.get('githubToken'))
            rw.pop('githubToken', None)  # o token NUNCA sai do arquivo
            return self._json(200, {'contratante': cfg.get('contratante', {}), 'hostgator': hg, 'railway': rw})
    ...
    def do_PUT(self):
        if self.path.split('?')[0] == '/api/config':
            ...
            if 'contratante' in corpo or 'hostgator' in corpo or 'railway' in corpo:
                ...
                if 'railway' in corpo:
                    rw = cfg.get('railway', {})
                    for k, v in corpo['railway'].items():
                        if not isinstance(v, str): continue
                        if k == 'githubToken' and v == '': continue  # em branco = mantém o atual
                        rw[k] = v
                    cfg['railway'] = rw
