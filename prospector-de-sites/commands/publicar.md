...
4. **Publique seguindo a skill correspondente**:
   ...
   - **Railway** (`deploy-railway`): copie a página e a capa para `[repoPath]/public/[pastaBase]/[slug]/` e tente `git add`/`commit`/`push` direto do sandbox (git sobre HTTPS costuma passar onde FTP não passa); se falhar, peça o duplo clique no `publicar-agora.command`/`.bat` da pasta conectada. Aguarde ~1-2 min (build do Railway).
